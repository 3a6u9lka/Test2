    ...
    public abstract class CodeExtension : ICodeExtension
    {
        ...
        public virtual void Process(CodeNamespace code, XmlSchema schema)
        {
            ...
            foreach (var type in types)
            {
                ...
                if (GeneratorContext.GeneratorParams.Miscellaneous.ExcludeImportedTypes)
                {
                    //if the typeName is NOT defined in the Included schemas, skip it.
                    if (!ContainsImportedTypeName(schema, type))
                    {
                        code.Types.Remove(type);
                        continue;
                    }
        ...
        private bool ContainsImportedTypeName(XmlSchema schema, CodeTypeDeclaration type)
        {
            //имена всех типов, содержащихся в файле, начинаются с имени схемы
            var starts = true;
            
            foreach (var itemRaw in schema.Items)
            {
                //Исключаем только элементы
                if (!(itemRaw is XmlSchemaElement)) return true;

                var item = (XmlSchemaElement) itemRaw;
                if (!type.Name.StartsWith(item.Name)) starts = false;
            }

            return starts && ContainsTypeName(schema, type);
        }
        ...
        protected static CodeTypeMember GetShouldSerializeMethod(string propertyName)
        {
            ...
            CodeExpression value = new CodePrimitiveExpression(true);
         
            var statement = new CodeMethodReturnStatement(value);
