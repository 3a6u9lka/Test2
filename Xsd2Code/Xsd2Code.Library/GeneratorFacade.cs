        ...
        private Result Process(string outputFilePath)
        {
            ...
            try
            {
                try
                {
                    ...
                    //save source encoding
                    Encoding srcEncoding;
                    using (var inputStream = File.Open(GeneratorContext.GeneratorParams.InputFilePath, FileMode.Open,
                        FileAccess.Read, FileShare.ReadWrite))
                    using (var reader = new XmlTextReader(inputStream))
                    {
                        reader.Read();
                        srcEncoding = (Encoding) reader.Encoding.Clone();
                    }

                    using (var outputStream = new StreamWriter(outputFilePath + ".tmp", false, srcEncoding))
